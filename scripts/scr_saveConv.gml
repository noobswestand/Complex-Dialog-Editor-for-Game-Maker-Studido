...
while(_str!=""){
    ...
    if string_count("conv(",st)>0{
        
        var bstr="";
        
        o0=-1 o1=-1
        if string_count("if ",st)==0{
            var st2=string_delete(st,1,string_length("conv("))
            o0=real(string_copy(st2,0,string_pos(",",st2)-1));
            o1=real(string_replace(string_copy(st2,string_pos(",",st2)+1,string_pos(")",st2)-1),")",""));
        }else{
            bstr=string_copy(st,1,string_pos("conv(",st)-1)
            var st2=string_delete(st,1,string_length("conv(")-1+string_pos("conv(",st))
